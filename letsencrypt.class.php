...
class letsencrypt
{
    ...
    /**
     * i just want JSON! / 过滤JSON
     * Author: Prk
     * 
     * The JSON returned by Letsencrypt is not a qualified JSON string.
     * Always add numbers at the end.
     * Use regex to filter the JSON-only part and convert it to an array.
     */
    private function onlyJson($unrealJson)
    {
        preg_match('/\b{.+}\b/', $unrealJson, $json);
        return $json[0];
    }
    ...
    public function orderDownload($id, $type, $day)
    {
        ...
        if (is_null(json_decode($res))) return json_decode(
            $this->onlyJson(
                $res
            ),
            true
        ); // 如果是 JSON （失败）那就直接返回 JSON
